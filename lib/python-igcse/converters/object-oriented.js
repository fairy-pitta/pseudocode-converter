...
const convertMethodAssignment = (line, indentation, state) => {
    ...
    if (method === 'upper') {
        state.declarations.add(variable);
        return {
            convertedLine: `${indentation}${variable} ${constants_1.OPERATORS.ASSIGN} UPPER(${object})`,
            blockType: null
        };
    }
    if (method === 'lower') {
        state.declarations.add(variable);
        return {
            convertedLine: `${indentation}${variable} ${constants_1.OPERATORS.ASSIGN} LOWER(${object})`,
            blockType: null
        };
    }
    // Capitalize method name for pseudocode convention
